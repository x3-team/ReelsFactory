...
retenza (590–2490₽, удержание, нет суфлёра). Mini App «профиль → сценарий →
...
ключа не врём, автор кидает ссылки/Insights. Не обещаем рост подписчиков и не
строим фабрику AI-видео.
...
- Корпус 16 публичных хендлов владельца — `src/lib/test-corpus.ts`. Без
  `APIFY_TOKEN`/`RAPIDAPI_KEY` live не гонялся (`pnpm test:corpus` → HARD STOP).
  Демо по корпусным `@` запрещено, даже с `ALLOW_MOCK_PROFILE`.
- Живых ключей скрейпа/AI/YooKassa в этой среде нет — ни один из 16 аккаунтов
  не разобран. Не утверждаем, что видели их ролики.
