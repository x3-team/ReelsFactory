...
Обновлён: 13.08.2026 · ветка `cursor/cis-content-bomb-features-fc8c` ·
...
| Ключ | Без него |
| --- | --- |
| `APIFY_TOKEN` (или `RAPIDAPI_KEY`) | профиль берётся из демо-данных — скрипт остановится |
| `AITUNNEL_API_KEY` | стратегия и сценарии из моков |

Ловушка, из-за которой скрипт и падает: с ключом AI, но без ключа скрейпинга
модель отработает вживую по выдуманным фактам, и отчёт будет выглядеть настоящим.
Демо-прогон разрешается явно: `ALLOW_MOCK_PROFILE=true`.
