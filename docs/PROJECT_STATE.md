...
читаемый отчёт (стратегия, сценарии, суфлёр, съёмочный день) в `/tmp`. Реальные
данные включаются ключами `APIFY_TOKEN` и `AITUNNEL_API_KEY` в `.env`; без них
прогон идёт на демо-данных, и это видно в шапке отчёта.
