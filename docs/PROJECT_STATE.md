...
**Черновик / не для демо заказчику как live:** живой LLM/Whisper (нет
`AITUNNEL_API_KEY`); YouTube; Agency-клиенты без своих ссылок; Студия;
пакет площадок как обещание кросспоста; ЮKassa без реквизитов.
...
- Корпус 16 прогнан 15.08.2026 с `APIFY_TOKEN`, без `AITUNNEL_API_KEY`
  (`HARD_STOP_NO_AITUNNEL` — LLM не мокали). 10 live-скрейпов, 5 YouTube 400,
